		...
		class tvr_microthemer_admin {
			...
			function get_validation_response($email, $context = 'unlock'){
				...
				if ( strpos($responseString, '{') !== false ){
					$response = json_decode($responseString, true);
					$validation = !empty($response['unlock']) ? $response['unlock'] : true;
				} else {
		            ...
		            if ($context == 'unlock'){
			            if (!$this->preferences['buyer_validated']) { // not already validated
				            $this->log(
					            ...
					            '<p>' . esc_html__('We activated your precious random letter. Microthemer\'s full program features have been unlocked!', 'microthemer') . '</p>',
					            'notice'
				            );
			            } else {
